...
public class InteractionScanner implements Scanner {
    ...
    private final ClientPlayerEntity player = Objects.requireNonNull(client.player);
    private final ClientWorld world = Objects.requireNonNull(client.world);
    private final ClientPlayerInteractionManager interactionManager = Objects.requireNonNull(client.interactionManager);
    /* ------ */
    ...
    private void initPlacementBlocks() {
        ...
        for (String region : schematic.getAreas().keySet()) {
            ...
            for (BlockPos pos : blockEntities.keySet()) {
                BlockPos worldPos = LocalPlacementPos.getWorldPos(pos, region, placement);
    ...
    private Set<BlockPos> getNearbyContainers() {
        Vec3d camera = getEyesPos(player);
        float reach = interactionManager.getReachDistance();
        List<BlockPos> positions = placement != null ? getPlacementContainers(camera, reach) : getWorldContainers(camera, reach);
    ...
    private List<BlockPos> getPlacementContainers(Vec3d camera, float reach) {
        ...
        return getAvailable(camera, reach, Iterables.filter(waiting, box::contains));
    }
    ...
    private Optional<BlockPos> validatePos(BlockPos pos, Vec3d camera) {
        ...
        if (state.isAir() || cache.containsKey(pos) || ContainerUtils.validateContainer(pos, state).isEmpty())
            return Optional.empty();

        if (state.getBlock() instanceof ChestBlock) {
            if (!player.isSpectator() && !ContainerUtils.isChestAccessible(world, pos, state))
                return Optional.empty();
        ...
        if (state.getBlock() instanceof ShulkerBoxBlock) {
            return player.isSpectator() || ContainerUtils.isShulkerBoxAccessible(world, pos, state)
    ...
    private void register(BlockPos pos, long tick) {
        ...
        LinkedStorageEntry entry = placement == null ? new LinkedStorageEntry(pos, worldInv, null) : PlacementContainerAccess.getEntry(pos, state, worldInv);
        cache.put(pos, entry);
        Optional<Color4f> color = entry.validate();
        Color4f red = new Color4f(1, 0, 0, 1);
        color.ifPresent(it -> OutlineRenderer.add(world, red.intValue, camera -> pos.getSquaredDistance(camera) <= 32 * 32, pos));
        waiting.remove(pos);
        ...
        interact(pos, getEyesPos(player));
    }
    ...
    private void interact(BlockPos pos, Vec3d player) {
        ...
        interactionManager.interactBlock(client.player, Hand.MAIN_HAND, hit);
    }

    @Override
    public void tick() {
        if (!running || (placement != null && waiting.isEmpty()))
            return;
