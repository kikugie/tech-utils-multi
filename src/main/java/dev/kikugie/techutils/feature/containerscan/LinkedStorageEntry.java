...
public class LinkedStorageEntry {
    ...
    public Optional<Inventory> getWorldInventory() {
        return Optional.ofNullable(worldInventory);
    }
    ...
    public Optional<Inventory> getPlacementInventory() {
        return Optional.ofNullable(placementInventory);
    }
    ...
    public Optional<Color4f> validate() {
        if (worldInventory == null || placementInventory == null)
            return Optional.empty();

        int type = 0;
        for (int i = 0; i < worldInventory.size(); i++) {
            ItemStack world = worldInventory.getStack(i);
            ItemStack schem = placementInventory.getStack(i);
