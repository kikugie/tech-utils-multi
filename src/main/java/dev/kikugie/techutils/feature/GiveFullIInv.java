...
public class GiveFullIInv {
    ...
    public static boolean containerHasItems(ItemStack container) {
        if (container.getItem() instanceof BlockItem blockItem && blockItem.getBlock() instanceof BlockEntityProvider) {
            if (!container.getOrCreateNbt().contains("BlockEntityTag", NbtElement.COMPOUND_TYPE))
                return false;

            BlockEntity blockEntity = BlockEntity.createFromNbt(BlockPos.ORIGIN, blockItem.getBlock().getDefaultState(), container.getSubNbt("BlockEntityTag"));
